...
public class QuizLeaderboard {
    ...
    public static void main(String[] args) {
        ...
        for (int poll = 0; poll < 10; poll++) {
            try {
                ...
                if (response.statusCode() != 200) {
                    System.err.println("Failed to fetch data for poll " + poll + ": HTTP " + response.statusCode());
                    continue;
                }
                ...
                while (eventMatcher.find()) {
                    ...
                    try {
                        ...
                        String scoreStr = extractNumber(eventJson, "score");
                        int score = Integer.parseInt(scoreStr);
                        
                        if (rowId.isEmpty() || participant.isEmpty()) {
                            System.err.println("Skipping invalid event: " + eventJson);
                            continue;
                        }
                        ...
                        System.err.println("Invalid score in event: " + eventJson);
                    }
                }
            } catch (Exception e) {
        ...
        for (int i = 0; i < sortedList.size(); i++) {
            ...
            leaderboardJson.append(String.format("{\"participant\":\"%s\",\"totalScore\":%d}", escapeJson(entry.getKey()), entry.getValue()));
